...
Written 2026-08-24 as step zero of a reorganization: the point is to make the
...
Scope: `tools/`, `src/`, `models/` — 21 modules, ~5,250 lines, 104 public
functions. Not covered: `tools/tests/` (gitignored, local-only), `experiments/*/`
(configs + data, no code), `derivations/` (LaTeX).
...
flowchart TB
    ...
    subgraph check["Checkpoint 0.4 — are the error bars real?"]
        CC["check_coverage.py<br/>arms: planted / planting / rate / wilson"]
    ...
    subgraph plan["Planning & verification"]
        AT["allocation_table.py<br/>precision vs wall clock"]
        VP["verify_prediction.py<br/>predicted vs real seconds"]
    end
...
| `cost_model.py` | 166 | `budget tool`, `statistical tool` | Cost exponent $d$: pure power law and the affine $a + b\,i^d$ that fixed the small-scale regime. Timing aggregators + median CI. | `loglog` |
...
| `persistence.py` | 151 | `tool` | Run directories, `samples.npz` vs chunked `samples/`, metadata sidecars, content hashing. | — |
| `models.py` | 67 | `tool` | `ModelSpec` registry. Pure importer — simulation lives in `models/`. | `srw`, `synthetic` |
...
| module | L | tags | purpose | depends on |
|---|---|---|---|---|
| `generate.py` | 354 | `experiment`, `tool` | Draw samples per a recipe. Allocation rules (`snr`/`neyman`), chunked output for large $n$. | `allocation`, `models`, `persistence` |
| `measure_cost.py` | 198 | `experiment`, `budget tool` | **Experiment A**: time `simulate()` per scale, fit $d$. | `cost_model`, `loglog`, `models`, `persistence` |
| `estimate_omega1.py` | 196 | `experiment`, `statistical tool` | **Experiment B**: $\omega_1$, $a_1$, $\gamma$, $a_0$ from one run. | `correction`, `loglog`, `persistence` |
| `allocation_experiment.py` | 384 | `experiment`, `budget tool` | **Experiment C**: sweep $m_0\times B$; paired `prop:opt` / tuned arms. | `allocation`, `loglog`, `models`, `persistence` |
| `check_coverage.py` | 433 | `experiment`, `statistical tool` | **Checkpoint 0.4**: four arms — `planted`, `planting`, `rate`, `wilson`. Holds srw's exact moments as *scoring* truth. | `coverage`, `wilson`, `correction`, `allocation_experiment`, … |
| `allocation_table.py` | 627 | `budget tool`, `statistical tool` | Planning: precision vs wall clock. Discovers run groups, pools replicates, reports provenance. **Largest module — see §5.** | `allocation`, `correction`, `persistence` |
| `verify_prediction.py` | 188 | `experiment`, `budget tool` | Runs tuned ladders for real; predicted vs measured seconds and RMSE. | `allocation`, `allocation_table`, `loglog`, … |
| `plot_allocation.py` | 323 | `plot tool` | Experiment C's two panels: $m_0$ tradeoff, and measured vs predicted decay rate. | `allocation_experiment`, `allocation_table`, `coverage` |
| `plot_loglog.py` | 159 | `plot tool` | Raw data + estimator comparison for any run. | `loglog`, `loglog_plot`, `models`, `persistence` |
| `plot_cost.py` | 86 | `plot tool` | Cost-probe timings. | `loglog_plot` |

### `models/` — the simulated objects

| module | L | tags | purpose |
|---|---|---|---|
| `srw.py` | 129 | `model` | $\lvert S_k\rvert$ for a $\pm1$ random walk. Deliberately $\Theta(k)$: integer-style stepping, **not** `binomial`, so it stays a percolation stand-in with real cost. |
| `synthetic.py` | 123 | `model` | Planted eq. (232) generator with arbitrary $(a_j,\omega_j)$ and pluggable noise. The only model with a `target_fn`. |
...
<details>
<summary><b>tools/</b> (37 functions)</summary>
...
<details>
<summary><b>src/</b> (28 functions)</summary>
...
none of them acted on yet.

1. **`src/allocation_table.py` is 627 lines and does three jobs**: discovering
...
2. **`src/check_coverage.py` holds srw's exact moments.** Deliberate — truth must
...
3. **The `tool` / `experiment` split is clean, but `src/` mixes two kinds of
   script**: generators of data (`generate`, `measure_cost`,
   `allocation_experiment`) and readers of it (`estimate_omega1`,
   `allocation_table`, `plot_*`). A `src/run/` vs `src/report/` split would make
   the DAG in §1 visible in the filesystem.
...
6. **Experiment A is the only dashed box.** Either run it as the cross-check
   ...
   — right now `measure_cost.py` is doing A's job without A's name.
