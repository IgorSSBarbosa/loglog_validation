...
| `models.py` | 105 | `tool` | `ModelSpec` registry. Pure importer — simulation lives in `models/`. `batched_cost` marks the `*_gpu` models, which are timed by the slope in $n$. | `srw`, `rwre`, `erw`, `synthetic`, `percolation2d`, `percolation_tau`, `percolation_zd`, `percolation_tau_zd` |
