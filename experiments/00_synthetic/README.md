...
**Status: generator built (`generator.py`); checkpoint 0.2 (closed-form estimator) done
via `tools/tests/test_loglog.py`; how the remaining checkpoints (0.1, 0.3-0.5) get
verified going forward is still an open question** (the earlier `run_checkpoint_0_1.py`
acceptance script was removed — see PLAN.md discussion; 0.2 was small enough to settle
directly as ordinary pytest unit tests instead of waiting on that decision). See
`PLAN.md` (repo root) "Phase 0" for the full checkpoint table with acceptance criteria —
reproduced here for convenience, kept in sync manually.
...
$(a_j,\omega_j)$ pairs, not just a single term. Output metadata now also records
`timing_seconds` (wall-clock draw time per scale) — not consumed by anything yet, but
the raw material for a future meta-log-log plot of $\mathrm{cost}(i)$ vs $i$ to estimate
the article's cost-model exponent $d$. `generate(..., progress=True)` (the CLI's default,
off by default for library callers) prints a one-line-per-scale update to stderr; the
CLI's summary table also gains an `elapsed_ms` column.
...
— see `tools/README.md` for what's implemented (as of checkpoint 0.2, also the article's
own closed-form weighted estimator, not currently wired into `compare_methods` since it
requires scales on a consecutive $\rho^k$ grid) and what's still open.
